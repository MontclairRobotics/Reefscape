 ...
 */
public class AlignToTagCommand3 extends Command {
     ...
     */
    private AlignToTagCommand3(Limelight camera, boolean useTagSize) {
        this.camera = camera;
        this.useTagSize = useTagSize;

        xController = new PIDController(5, 0, .1);
        xController.setTolerance(0.02); // 2cm
        yController = new PIDController(5, 0, .05);
     ...
     * @param useTagSize If true, uses the size of the tag to determine distance, otherwise uses the height of the tag
     */
    public AlignToTagCommand3(Limelight camera, TagOffset offset, boolean useTagSize) {
     ...
     * @param useTagSize If true, uses the size of the tag to determine distance, otherwise uses the height of the tag
     */
    public AlignToTagCommand3(Limelight camera, int tagId, TagOffset offset, boolean useTagSize) {
        this(camera, useTagSize);
    ...
    public void initialize() {
        ...
        double xSetpoint = tagOffset.getXOffsetM() - camera.getCameraOffsetX();
    ...
    public void execute() {
        ...
        double thetaSpeed = thetaController.calculate(currentTheta) * 2;
