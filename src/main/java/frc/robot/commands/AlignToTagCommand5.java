 ...
 */
public class AlignToTagCommand5 extends Command {
     ...
     */
    private AlignToTagCommand5(Limelight camera, boolean useTagSize, boolean isOffset) {
        this.camera = camera;
        ...
        yController = new PIDController(2.5, 0, .05);
     ...
     * @param useTagSize If true, uses the size of the tag to determine distance, otherwise uses the height of the tag
     */
    public AlignToTagCommand5(Limelight camera, TagOffset offset, boolean useTagSize, boolean isOffset) {
        this(camera, useTagSize, isOffset);
     ...
     * @param useTagSize If true, uses the size of the tag to determine distance, otherwise uses the height of the tag
     */
    public AlignToTagCommand5(Limelight camera, int tagId, TagOffset offset, boolean useTagSize, boolean isOffset) {
        this(camera, useTagSize, isOffset);
    ...
    public void initialize() {
        ...
        // If we are in offset mode, point at tag, otherwise align to reef
        double thetaSetpoint = isOffset ? 0 : targetRotation.getRadians();
    ...
    public void execute() {
        ...
        if (false) { // (Math.abs(targetRotation.getRadians() - currentTheta) > Units.degreesToRadians(10)) { // (!thetaController.atSetpoint() || camera.getTagID() != tagId) {
            ...
            if (!useTagSize) {
                ...
                x = Math.abs(REEF_APRIL_TAG_HEIGHT - camera.getCameraHeightMeters()) / Math.tan(Units.degreesToRadians(ty - camera.getCameraAngle()));
                // y distance to tag is x distance * tan(x angle to tag)
                y = x * Math.tan(Units.degreesToRadians(tx));
            } else {
                ...
                x = Math.abs(Units.inchesToMeters(8.75) / Math.tan(yAngle));
                // y distance to tag is x distance * tan(x angle to tag)
                y = x * Math.tan(Units.degreesToRadians(tx));
            ...
            double xSetpoint = tagOffset.getXOffsetM() - camera.getCameraOffsetX() + (isOffset ? 0.3 : 0.0);
            double ratio = Math.min((x - xSetpoint) / 3, 1);
            if (ratio < 0.1) {
                ratio = 0.0;
            }
            if (!isOffset) {
            ...
            double dynamicXSetpoint = xSetpoint + ratio * x/2;
            ...
            double currentTheta = (ratio > .2) ? Units.degreesToRadians(tx) : robotPose.getRotation().getRadians();
    ...
    public boolean isFinished() {
        ...
        boolean isFinished =  atSetPoint || !camera.hasValidTarget() || (camera.getTagID() != tagId);
        Logger.recordOutput("AlignToTag/tagId", camera.getTagID());
        Logger.recordOutput("AlignToTag/tagId2", tagId);
