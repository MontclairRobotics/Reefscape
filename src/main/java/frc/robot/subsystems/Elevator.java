...
public class Elevator extends SubsystemBase {
    ...
    private static final double PERCENT_HEIGHT_DEADBAND = 0.004; //(As a decimal not percent) Ex. 0.04 is 4%
                                                     //The deaband used in manual control for percent height                                                 
    ...
    public void joystickControl() {
        ...
        // voltage = accelerationLimiter.calculate(voltage); //TODO: uncomment
        ...
        if (voltage < 0) {
            if (percentHeight <= 0.04) {
                accelerationLimiter.reset(0);
                System.out.println("At bottom!---------------------");
            }
        }

        voltage = MathUtil.clamp(voltage, -12 * (Math.pow( (MathUtil.applyDeadband(percentHeight, PERCENT_HEIGHT_DEADBAND) * (100.0 / SLOW_DOWN_ZONE)), 3.2)) - SLOWEST_SPEED, /*lowest voltage allowed*/
         12 * (Math.pow( (MathUtil.applyDeadband(1 - percentHeight, PERCENT_HEIGHT_DEADBAND)) * (100.0 / SLOW_DOWN_ZONE), 3.2)) + SLOWEST_SPEED) /*highest voltage allowed*/;
        ...
        //Puting it to the power of 3 (or somthing else) makes the slowdown more noticable
        //The formula is 12 * (percentHeight * 100/7)^3
